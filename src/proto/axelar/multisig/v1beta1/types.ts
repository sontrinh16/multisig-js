...
import { isSet, bytesFromBase64, base64FromBytes, DeepPartial, isObject, Long } from "@osmonauts/helpers";
  ...
  toJSON(message: KeygenSession): unknown {
    ...
    if (message.isPubKeyReceived) {
      Object.entries(message.isPubKeyReceived).forEach(([k, v]) => {
        obj.isPubKeyReceived[k] = bool.toJSON(v);
      });
    }
